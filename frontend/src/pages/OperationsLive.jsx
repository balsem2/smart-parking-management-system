...
export function PendingAccountsLive() {
  ...
  return <><Head title="Pending accounts" subtitle="Review staff account requests before activation." onRefresh={load} loading={loading} /><ErrorMessage error={error} /><div className="panel table-wrap"><table><thead><tr><th>ID</th><th>Username</th><th>Email</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead><tbody>{items.length ? items.map(item => <tr key={item.id}><td>{item.id}</td><td><strong>{item.username}</strong></td><td>{item.email}</td><td>{item.role}</td><td>{item.status}</td><td><div className="inline-actions"><button type="button" className="small-button" onClick={() => handleAction(item.id, 'approve')}>Approve</button><button type="button" className="small-button secondary" onClick={() => handleAction(item.id, 'reject')}>Reject</button></div></td></tr>) : <tr><td colSpan="6" className="empty">No pending accounts.</td></tr>}</tbody></table></div></>
}

export function VehiclesLive() {
  const [items, setItems] = useState([]); const [status, setStatus] = useState('ALL'); const [error, setError] = useState(''); const [loading, setLoading] = useState(false)
  const load = async () => { setLoading(true); try { setItems(await getVehicles()); setError('') } catch (err) { setError(err.message) } finally { setLoading(false) } }
  useEffect(() => { load() }, [])
  const shown = items.filter(item => status === 'ALL' || item.status === status)
  return <><Head title="Vehicles" subtitle="Manage vehicles permitted to access your parking facility." onRefresh={load} loading={loading} /><ErrorMessage error={error} /><div className="filters"><StatusFilter value={status} onChange={setStatus} options={['AUTHORIZED', 'VISITOR', 'VIP', 'BLACKLISTED']} /></div><div className="panel table-wrap"><table><thead><tr><th>ID</th><th>Plate</th><th>Owner</th><th>Vehicle</th><th>Status</th></tr></thead><tbody>{shown.length ? shown.map(item => <tr key={item.id}><td>{item.id}</td><td><strong>{item.plate_number}</strong></td><td>{item.owner_name || '—'}</td><td>{[item.brand, item.model].filter(Boolean).join(' ') || '—'}</td><td>{item.status}</td></tr>) : <tr><td colSpan="5" className="empty">{items.length ? 'No vehicles match this status.' : 'No vehicles found.'}</td></tr>}</tbody></table></div></>
}
